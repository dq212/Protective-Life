...
gulp.task("copyStatic", function() {
    gulp
        ...
        .pipe(rename("5528_PL_AprilLife_Banners_DentistWisdom-3_160x600.gif"))
...
gulp.task("imageCompress", ["copy", "sass"], function(done) {
    gulp
        .src([dist + "/*.{jpg,png,svg,gif}"].concat(uncompressedImgs))
        .pipe(
            image({
                pngquant: true,
                optipng: false,
...
gulp.task(
    ...
    function() {
        return gulp
            .src(dist + "/*")
            .pipe(zip("5528_PL_AprilLife_Banners_DentistWisdom-3_160x600.zip"))
            .pipe(gulp.dest("../HTML/"));
    }
);